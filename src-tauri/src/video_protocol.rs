...
const MAX_CHUNK_SIZE: u64 = 2 * 1024 * 1024; // 2 MB for Android (Avoid IPC OOM and Decoder errors)
...
fn decode_path(request: &Request<Vec<u8>>) -> Option<PathBuf> {
    ...
    let looks_unc = as_str.starts_with(r"\\");
    let looks_drive = bytes.len() >= 2 && bytes[1] == b':' && bytes[0].is_ascii_alphabetic();
    if as_str.starts_with('/') || looks_unc || looks_drive {
